...
@pytest.fixture
def truth():
    """A z-scored series with periodicity and autocorrelation."""
...
def test_nd_is_a_fixed_multiple_of_mae(truth):
    """ND / MAE is 1 / mean|y|, the same for any reconstruction of one series."""
...
def test_nrmse_is_a_fixed_multiple_of_rmse(truth):
    """nRMSE / RMSE is 1 / std(y), the same for any reconstruction of one series."""
...
def test_rmse_equals_mae_when_every_error_is_the_same(truth):
    """A constant offset makes the two coincide, which is the floor of RMSE / MAE."""
    pred = truth + 0.5
...
def test_rmse_over_mae_is_one_over_root_p_for_concentrated_error(truth, p):
    """With the error confined to a fraction p of positions, RMSE / MAE = 1 / sqrt(p)."""
...
def test_distribution_metrics_cannot_see_a_permutation(truth):
    """A permutation leaves the multiset unchanged, so WD, JSD and KLD stay at zero."""
...
def test_mean_difference_metrics_cannot_see_a_permutation(truth):
    """BA and CDT stay at zero under a permutation, which preserves the mean."""
...
def test_mean_difference_metrics_cannot_see_a_rescaling(truth):
    """BA and CDT stay at zero under an expansion around the mean."""
    mu = truth.mean()
...
def test_temporal_metrics_do_see_a_permutation(truth):
    """ACF and DTW do register a destroyed order, unlike the metrics above."""
...
def test_a_perfect_reconstruction_scores_perfectly(truth):
    """Every metric reads its best value against the truth itself."""
...
def test_lower_is_better_metrics_rise_with_noise(truth, metric):
    """Each lower-is-better metric increases with the noise level."""
...
def test_higher_is_better_metrics_fall_with_noise(truth, metric):
    """Each higher-is-better metric decreases with the noise level."""
