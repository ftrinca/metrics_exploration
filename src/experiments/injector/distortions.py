...
from metric_eval.experiments.injector.config import SPIKE_RATE
...
def damage_rmse(y_col: np.ndarray, idx: np.ndarray, values: np.ndarray) -> float:
    """Root mean squared error at the masked positions, in units of sigma.

    The second calibration target: a metric that serves as the target cannot
    react, so MAE and ND are read off the pass solved against this one.
    """
    sigma = gap_sigma(y_col, idx)
    if sigma <= 0.0:
        return 0.0
    return float(np.sqrt(np.mean((values - y_col[idx]) ** 2))) / sigma


# The solvable calibration targets, keyed by the CLI's --damage-metric.
DAMAGE_FUNCTIONS = {"mae": damage, "rmse": damage_rmse}
